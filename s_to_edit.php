...
<?php 
	if(isset($_GET['id'])){
		...
		$qry->execute();
		$qry->bind_result($id,$dbstudent_id, $dbdestination, $dbdeparture, $dbreturn,$dbpurpose,$status,$created_at,$diem,$remarks);
		$qry->store_result();
		$qry->fetch ();
	}
...
<section class="content">
	...
	<div class="box">
 	...
 	if(isset($_POST['btnUpdate'])){
 	 		$sql = "UPDATE s_travel_order SET student_id=?,destination=?,dreturn=?,departure=?,purpose=?,diem=?,remarks=? WHERE id=?";
 	 		$qry = $connection->prepare($sql);

 	 		$qry->bind_param("issssssi",$_POST['student_id'],$_POST['destination'],$_POST['return'],$_POST['departure'],$_POST['purpose'],$_POST['diem'],$_POST['remarks'],$_POST['id']);

 	 		if($qry->execute()) {
 	 		
 	 			echo '<meta http-equiv="refresh" content="0; URL=s_to_list.php?status=updated">';
 	 		}else{
