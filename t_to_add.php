...
<section class="content">
	...
	<div class="box">
				...
				<div class="col-md-6 ">
					<div class="form-group">
						...
						<select required name="teacher_id" class="form-control select2" style="width: 100%;">
							...
							<?php 
									$sql = "SELECT id,firstname,lastname,position,department FROM teacher";
									$qry = $connection->prepare($sql);
									$qry->execute();
									$qry->bind_result($id,$dbf,$dbl, $dbp, $dbd);
									$qry->store_result();
									while($qry->fetch ()) {
										echo"<option value='".$id."'>";
										echo $dbf." ".$dbl." - ".$dbp." , ".$dbd;
										echo"</option>";
									};
							?>
						</select>
		...
		<div class="pull-right" style="margin-top:5px">
			<a href="t_to_list.php" class="btn btn-danger"> Go Back</a>
 	...
 	if(isset($_POST['btnAdd'])){
 			...
 			$checkschedule = "SELECT departure,dreturn FROM t_travel_order WHERE teacher_id=? ";
 			$qry = $connection->prepare($checkschedule);
 			$qry->bind_param("i",$_POST['teacher_id']);
 			$qry->execute();
 			...
 			while($qry->fetch()){
 				...
 				$sql = "INSERT INTO t_travel_order(teacher_id,destination,dreturn,departure,purpose,status,created_at,diem,remarks) VALUES(?,?,?,?,?,?,NOW(),?,?)";
 				$status = "";
 				$qry = $connection->prepare($sql);
 				$qry->bind_param("isssssss",$_POST['teacher_id'],$_POST['destination'],$_POST['return'],$_POST['departure'],$_POST['purpose'],$status,$_POST['diem'],$_POST['remarks']);

 				if($qry->execute()) {
 				
 					echo '<meta http-equiv="refresh" content="0; URL=t_to_list.php?status=created">';
 				}else{
