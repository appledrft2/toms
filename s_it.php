...
<?php 
	if(isset($_GET['id'])){
		$sql = "SELECT * FROM s_travel_order WHERE id=?";
...
?>
<?php 
    $sql = "SELECT id,firstname,lastname,department FROM student WHERE id=?";
    $qry = $connection->prepare($sql);
...
<section class="content">
	...
	<div class="box">
		<h3 class="box-header text-right"><a onclick="window.open('s_it_print.php?id=<?php echo $_GET['id'] ?>', 
                         'newwindow', 
		...
		<div class="table-responsive">
			<table class="table table-bordered table-striped">
				...
				<tbody>
					<?php 
						$sql = "SELECT * FROM s_iterinary WHERE s_tv_id=?";
						$qry = $connection->prepare($sql);
						$qry->bind_param('i',$_GET['id']);
						$qry->execute();
						...
						while($qry->fetch ()) {
							...
							echo '<a href="s_ti_delete.php?id='.$id.'&s_tv_id='.$_GET['id'].'" ';?>onclick="return confirm('Are you sure?')"<?php echo 'class="btn btn-danger btn-sm"><i class="fa fa-remove"></i></a>';
							echo "</td>";
							echo "</tr>";
							echo "</form>";
						...
						$sum = $sum + $diem;
						$updatetotal = "UPDATE s_travel_order SET total=? WHERE id=?";
					...
					<form method="POST" action="#">
						<tr>
							<td><input type="hidden" name="s_tv_idadd" value="<?php echo $_GET['id'] ?>"></td>
...
if(isset($_POST['btnAdd'])){
	$sql = "INSERT INTO s_iterinary(s_tv_id,placetovisit,departure,arrival,meansoftrans,transallowed,total) VALUES(?,?,?,?,?,?,?)";
	$qry = $connection->prepare($sql);
	$qry->bind_param("issssss",$_POST['s_tv_idadd'],$_POST['placestovisitadd'],$_POST['departureadd'],$_POST['arrivaladd'],$_POST['meansoftraveladd'],$_POST['transallowedadd'],$_POST['transallowedadd']);

	if($qry->execute()){
		echo '<meta http-equiv="refresh" content="0; URL=s_it.php?id='.$_POST['s_tv_idadd'].'&status=created">';
	}
}if(isset($_POST['btnUpdate'])){
	$sql = "UPDATE s_iterinary SET placetovisit=?,departure=?,arrival=?,meansoftrans=?,transallowed=?,total=? WHERE id=?";
	$qry = $connection->prepare($sql);
	...
	if($qry->execute()){
		echo '<meta http-equiv="refresh" content="0; URL=s_it.php?id='.$_GET['id'].'&status=updated">';
	}
}
